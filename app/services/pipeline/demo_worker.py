...
def _render_scene(scene, output_path: Path, width: int, height: int, fps: int) -> None:
    ...
    try:
        for frame_index in range(total_frames):
            ...
            # Animated gradient-like bands for a polished, dependency-light demo.
            for y in range(height):
                ratio = y / max(1, height - 1)
                r = int(18 + 45 * ratio + 12 * math.sin(phase))
                g = int(24 + 55 * ratio + 10 * math.sin(phase + 1.5))
                b = int(38 + 70 * ratio + 14 * math.sin(phase + 3.0))
            ...
            draw.text((margin + 30, margin + 25), f"SCENE {scene.sequence}",
                      fill=(220, 230, 245), font=small_font)
            ...
            narration = scene.narration or scene.subtitle_text
            ...
            draw.rounded_rectangle(
                (margin, height - margin - 25, margin + progress, height - margin - 13),
                radius=6,
                fill=(245, 180, 70),
            )
            draw.text((width - margin - 120, height - margin - 58),
                      "DEMO RENDER", fill=(245, 200, 120), font=small_font)
...
class DemoGenerationWorker:
    """CPU-only end-to-end renderer used to validate the complete workflow."""
    ...
    def run(self, job_id: str) -> dict:
        ...
        try:
            ...
            self._update(db, job, status=JobStatus.RUNNING, stage="demo_video_render",
                         progress=35, message=f"Rendering {len(plan.scenes)} demo scenes",
                         scene_count=len(plan.scenes), llm_provider=llm_provider)

            clips: list[Path] = []
            for scene in plan.scenes:
                clip = job_dir / f"{scene.scene_id}.mp4"
                _render_scene(scene, clip, width=832, height=480, fps=16)
                clips.append(clip)
            ...
            self._update(db, job, status=JobStatus.RUNNING, stage="artifact_ready",
                         progress=90, message="Demo MP4 ready for review")
            ...
            self._update(
                ...
                message="Demo video ready; awaiting approval",
                final_video_path=str(final_path),
                model_id="demo-render-v1",
                duration_seconds=plan.total_scene_duration,
